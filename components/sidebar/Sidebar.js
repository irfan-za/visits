      ...
      >
        <nav className="w-3/4 space-y-6 px-2 pb-24 pt-5 sm:w-1/2 lg:w-full">
          <div className="mb-2 flex space-x-4 border-b border-gray-300 p-4 tracking-wider text-gray-400/80">
            <div className="h-10 w-10 rounded-full bg-primary sm:h-12 sm:w-12 text-xl font-bold flex justify-center items-center text-white">M</div>
                      ...
                      <Link href={link.href} legacyBehavior passHref>
                        <NavigationMenuLink
                          className={`${navigationMenuTriggerStyle()} ${
                            activeNav === link.activeNav ? "bg-primary text-primary-foreground" : ""
                          }`}
                        >
                          {link.icon}
